...
import { mkdir, copyFile, rm, stat, readdir, writeFile } from 'node:fs/promises';
...
/**
 * Point the app at the files just written.
 *
 * Metro resolves `require` at bundle time, so the app cannot conditionally
 * require weights that may not exist — a try/catch around it still breaks the
 * build. `assets/models/manifest.js` is the indirection: it is committed with
 * null entries and rewritten here.
 */
async function writeManifest() {
  const contents = `/**
 * Model asset manifest — generated by \`npm run prepare-model\`.
 *
 * Committed with null entries so the app builds with no weights installed;
 * see apps/mobile/src/ai/assets.ts for why the indirection is necessary.
 */
module.exports = {
  model: require('./model.onnx'),
  tokenizer: require('./tokenizer.json'),
  config: require('./config.json'),
};
`;
  await writeFile(join(OUTPUT_DIR, 'manifest.js'), contents, 'utf8');
