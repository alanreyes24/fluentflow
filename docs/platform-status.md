...
**Nothing has been prepared for a device.** There is no `apps/mobile/ios/`
directory, so `expo prebuild` has never run. There is no `eas.json`. There is no
app icon and no splash image: `apps/mobile/assets/` contains only the empty
`models/` directory, and `app.json` declares no `icon` or `splash` key, even
though [app/_layout.tsx:4](../apps/mobile/app/_layout.tsx#L4) calls
`SplashScreen.preventAutoHideAsync()`. There is no development build and no
...
**Order of work.** Icon and splash assets, then `expo prebuild`, then a
...
| No icon or splash | `grep -E "icon\|splash" apps/mobile/app.json`, no matches |
